...
@pytest.mark.parametrize(
    ...
    [
        pr.RollPass(
            roll=pr.Roll(groove=pr.CircularOvalGroove(r1=1, r2=5, depth=1)),
            gap=1,
            in_profile=pr.Profile.round(diameter=4),
        ),
        pr.RollPass(roll=pr.Roll(groove=pr.CircularOvalGroove(r1=1, r2=5, depth=1)), gap=1),
        pr.RollPass(
            roll=pr.Roll(groove=pr.CircularOvalGroove(r1=1, r2=5, depth=1)),
            gap=1,
        ...
        pr.RollPass(
            roll=pr.Roll(groove=pr.CircularOvalGroove(r1=1, r2=5, depth=1), nominal_radius=100),
            gap=1,
            in_profile=pr.Profile.round(diameter=4),
...
@pytest.mark.parametrize("orientation", [0, 90, "Horizontal", 45, -45, "Vertical"])
@pytest.mark.xfail(
    not importlib.util.find_spec("matplotlib") and not importlib.util.find_spec("plotly"),
    reason="no plotting backend available",
)
...
@pytest.mark.parametrize("orientation", [0, 180, "Y", "AntiY", -180])
@pytest.mark.xfail(
    not importlib.util.find_spec("matplotlib") and not importlib.util.find_spec("plotly"),
    reason="no plotting backend available",
)
