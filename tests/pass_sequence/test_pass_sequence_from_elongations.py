...
def test_pass_sequence_velocity_calculation_from_elongations(tmp_path: Path, caplog):
    ...
    with RollPass.OutProfile.width(width):
        ...
        try:
            sequence.solve_velocities_from_elongations(
                in_profile=in_profile, final_speed=final_speed, elongations=elongations
            )
        finally:
            print("\nLog:")
            print(caplog.text)
    ...
    assert np.isclose(velocities[0], 19.2307).all()
