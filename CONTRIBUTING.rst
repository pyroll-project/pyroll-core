...
The following is a set of guidelines for contributing to PyRolL, which is
hosted and maintained by the `PyRolL-Project Core Team`_
...
You can find all the PyRolL packages at our `github page`_ .
To create pull requests, you will need to become part of the
PyRolL organization. Please email us if you would like to join.
...
2. If your question relates to a bug in PyRolL, please briefly search the `issues page`_ and open a new labeled issue if you don't see anything related to your question there.
3. You can also contact us on stackoverflow with the tag 'PyRolL'. There we will answer your questions. For bug fixes, please open an issue on github, as mentioned before.
...
`What should I know before I get started?`_
  * `PyRolL developer meetings`_

..
 * `The structure of PyRolL`_
..
 * `The principles of PyRolL`_
...
License
=======
PyRolL is released as an open-source project under the BSD 3-Clause License.
...
-------------------------
If you are interested in discussing PyRolL's development, we encourage you to virtually
participate in the PyRolL developer meeting Mondays at 15:00 german time (GMT+2).
Contact us if you want to participate in the weekly virtual PyRolL developer meeting, become part of the PyRolL organization, and check the discussion page for details.
...
Check if you can reproduce the problem in the latest version of PyRolL.
...
the PyRolL repository by including the following information:
...
* Include information about the environment (OS, python version, how packages were installed) in which you were running PyRolL.
...
* Provide sample code that you would use to access the feature. If possible, include code for how you think the feature could be built into PyRolL's codebase. Include code snippets as markdown code blocks.

Your first code contribution
----------------------------

Unsure where to begin contributing to PyRolL? You can start by looking
...
PyRolL can be developed and tested locally. If you are using PyRolL to run an
external software package, e.g. VASP or LAMMPS, you might also need to install
those packages locally to run certain integration tests in PyRolL.

To get the developmental (git) version of PyRolL,

.. code-block::

  git clone https://github.com/PyRolL/PyRolL.git
  conda env update --name PyRolL_dev --file PyRolL/.ci_support/environment.yml
  conda activate PyRolL_dev
  conda install conda-build
  conda develop PyRolL
  
**Deploy development version to a managed environment**

If you want to use a development version of PyRolL in a managed environment where a version of PyRolL is
already installed outside of your control (e.g. on the cmti/cmfe cluster), you can still preload a local
checkout of the repo, while using the dependencies already installed.  Assuming PyRolL and dependencies
...
.. code-block::
  ...
  export PYTHONPATH="$HOME/software/PyRolL:$PYTHONPATH"
...
Keep in mind that to run the tests your repository needs to be inside your PyRolL project folder and you need to have 
...
Please follow `PEP8 conventions`_ for all python code added to PyRolL. Pull
...
your pull request does not decrease PyRolL's overall coverage. This
...
under PyRolL/docs/source.

Notebooks created to exemplify features in PyRolL are very useful, and
can even be used as integration tests. If you have added a major feature,
consider creating a notebook to show its usage under PyRolL/notebooks/.
...
We use the following tags to organize PyRolL Github issues
and pull requests:
...
The build status for PyRolL and all sub packages are given below

.. image:: https://coveralls.io/repos/github/PyRolL/PyRolL/badge.svg?branch=main
    :target: https://coveralls.io/github/PyRolL/PyRolL?branch=main
    :alt: Coverage Status

.. image:: https://api.codacy.com/project/badge/Grade/c513254f10004df5a1f5c76425c6584b
    :target: https://app.codacy.com/app/PyRolL-runner/PyRolL?utm_source=github.com&utm_medium=referral&utm_content=PyRolL/PyRolL&utm_campaign=Badge_Grade_Settings
    :alt: Codacy Badge

.. image:: https://anaconda.org/conda-forge/PyRolL/badges/latest_release_date.svg
    :target: https://anaconda.org/conda-forge/PyRolL/
    :alt: Release_Date

.. image:: https://travis-ci.org/PyRolL/PyRolL.svg?branch=main
    :target: https://travis-ci.org/PyRolL/PyRolL
    :alt: Build Status

.. image:: https://ci.appveyor.com/api/projects/status/wfdgqkxca1i19xcq/branch/main?svg=true
    :target: https://ci.appveyor.com/project/PyRolL-runner/PyRolL/branch/main
    :alt: Build status

PyRolL release distribution
---------------------------

.. image:: https://anaconda.org/conda-forge/PyRolL/badges/downloads.svg
    :target: https://anaconda.org/conda-forge/PyRolL/
    :alt: Downloads

PyRolL is released through `conda-forge`_ and  `pip`_. 
Both packages are created automatically and maintained with every new release of PyRolL. In order to use these distributions simply use the following command for conda::
   conda install -c conda-forge PyRolL
In order to use the pip distribution use::
   pip install PyRolL
Just like each other commit to the master branch the tagged releases are pushed to pypi.org (https://pypi.org/project/PyRolL/#history)::
The major difference for pypi (pip) is that installing pre-release versions is possible using the `--pre` flag::
   pip install --pre PyRolL
...
This step is done manually and important to trigger all the following steps. Tag can be created under https://github.com/PyRolL/PyRolL/tags. 
...
4. Docker images
  The docker images are maintained manually and therefore not updated with every release. The docker images are build using the conda packages and can be found in different variants under https://github.com/PyRolL/docker-stacks
5. Graphical installer
  The graphical installer is also maintained manually and not updated as frequently and can be found at https://github.com/PyRolL/PyRolL-installer.

GitHub Workflows
-----------------------------
The `GitHub-Action-Workflows`_ are triggered at different occasions (eg. creating commit, push to master):

* UpdateDependabotPR.yml: https://github.com/PyRolL/PyRolL/blob/main/.github/workflows/UpdateDependabotPR.yml

* codeql-analysis.yml: https://github.com/PyRolL/PyRolL/blob/main/.github/workflows/codeql-analysis.yml

* deploy.yml: https://github.com/PyRolL/PyRolL/blob/main/.github/workflows/deploy.yml

* docs.yml: https://github.com/PyRolL/PyRolL/blob/main/.github/workflows/docs.yml

* notebooks.yml: https://github.com/PyRolL/PyRolL_base/blob/main/.github/workflows/notebooks.yml

* pypicheck.yml: https://github.com/PyRolL/PyRolL_base/blob/main/.github/workflows/pypicheck.yml
...
In this example, we want a custom job class `ProtoMD` from the module `PyRolL_contrib`:

.. code-block::

  from PyRolL import Project
  import PyRolL_contrib  # only if importing a custom job class
...
Make sure that the `__init__.py` of your module (here, `PyRolL_contrib`) initializes the job class in the following format:

.. code-block::

  from PyRolL import Project
  from PyRolL_base.job.jobtype import JOB_CLASS_DICT

  # Make classes available for new PyRolL version
  JOB_CLASS_DICT["ProtoMD"] = "PyRolL_contrib.protocol.compound.md"  # the path of your job class
...
.. code-block::
  ...
  >>> "<class 'PyRolL_contrib.protocol.compound.md.ProtoMD'>"
...
In **a new notebook** , load the job that you just saved, using its job id. You may or may not import the module (here, `PyRolL_contrib`):

.. code-block::

  from PyRolL import Project
  # we do not import PyRolL_contrib here, becasue it should not be necessary
...
.. _github page: https://github.com/PyRolL
.. _issues page: https://github.com/PyRolL/PyRolL/issues
.. _FAQ page: https://github.com/PyRolL/PyRolL/docs/source/faq.html
.. _bugs: https://github.com/PyRolL/PyRolL/issues?q=is%3Aopen+is%3Aissue+label%3A%22bug%22
.. _Good first issues: https://github.com/PyRolL/PyRolL/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22
.. _Help wanted issues: https://github.com/PyRolL/PyRolL/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22
...
.. _conda-forge: https://anaconda.org/conda-forge/PyRolL_base
.. _pip: https://pypi.org/project/PyRolL-base/
.. _Git-Tag-Guide: https://git-scm.com/book/en/v2/Git-Basics-Tagging
.. _Deploy-Workflow: https://github.com/PyRolL/PyRolL_base/blob/main/.github/workflows/deploy.yml
.. _GitHub-Action-Workflows: https://github.com/PyRolL/PyRolL_base/tree/main/.github/workflows
.. _dependabot: https://github.com/dependabot
.. _setup.ctg file: https://github.com/PyRolL/PyRolL/blob/main/setup.cfg
.. _notebooks folder: https://github.com/PyRolL/PyRolL_base/tree/main/notebooks